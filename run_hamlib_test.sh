...
python -m hamlib.hamlib_simulation_benchmark "${appargs[@]}" -g SpinOperator 2>&1 | tee hamlib.log
...
srun -n $NUM_GPUS python -m mpi4py -m hamlib.hamlib_simulation_benchmark "${appargs[@]}" -g SpinOperator 2>&1 | tee hamlib.log
...
python -m hamlib.hamlib_simulation_benchmark "${appargs[@]}" -g simple 2>&1 | tee hamlib.log
...
srun -n $NUM_GPUS python -m mpi4py -m hamlib.hamlib_simulation_benchmark "${appargs[@]}" -g simple -pm mpi 2>&1 | tee hamlib.log
